...
class Handlers
{
	...
	*/
	...
	function DoRemoveSession(oSessions: Session[]) {
		...
		file = fso.OpenTextFile(filePathForRemoveSession,2 , true, true);
		file.writeLine("\n");
		file.writeLine("Request url: " + oSessions[0].url);
		file.writeLine("Request header: " + oSessions[0].oRequest.headers);
		...
		s.Run("python F:\\test\\study\\ApiTests-master\\fiddlerRemoveSession.py");
	}

	public static ContextAction("Add Session from disk")
	function DoAddSession(oSessions: Session[]) {
		...
		var file;
		oSessions[0].utilDecodeResponse();
		file = fso.OpenTextFile(filePathForAddSession,2 , true, true);
		file.writeLine("\n");
		file.writeLine("Request url: " + oSessions[0].url);
		file.writeLine("Request header: " + oSessions[0].oRequest.headers);
		file.writeLine("Request body: " + oSessions[0].GetRequestBodyAsString());
		file.writeLine("Response code: " + oSessions[0].responseCode);
		file.writeLine("Response body: " + oSessions[0].GetResponseBodyAsString());
		file.writeLine("Session end" + "\r\n");
		file.close();
		//js 调用cmd执行python脚本进行删除
		var s = new ActiveXObject("WScript.Shell");
		s.Run("python F:\\test\\study\\ApiTests-master\\AddSession.py");
	}

	//自定义参数设置
	public static var filterUrl = "www.senbaba.cn";
	...
	*/
	...
	static function OnBeforeResponse(oSession: Session) {
		...
		//过滤无关请求，只关注特定请求
		if (oSession.fullUrl.Contains(filterUrl))
		{
			var responseBody = oSession.GetResponseBodyAsString();
			var url = oSession.url;
			// 首先存储Response code不等于200的情况，该请求必定服务器发生错误。
			if (oSession.responseCode != 200){
				file = fso.OpenTextFile(filePathForErrorResponse,8 ,true, true);
				file.writeLine("\n");
				file.writeLine("Request url: " + url);
				file.writeLine("Response code: " + oSession.responseCode + "\r\n");
				file.close();
				oSession["ui-color"] = "blue"
			} else if (responseBody.IndexOf("\"s\":1") != 1){
				...
				file.writeLine("Response code: " + oSession.responseCode);
				file.writeLine("Response body: " + responseBody);
				file.writeLine("Session end" + "\r\n");
				file.close();
				oSession["ui-color"] = "blue"
			} else {
				oSession.utilDecodeResponse();
				//按接口名保存已经请求接口的所有信息 高级
				file = fso.OpenTextFile(filePath + url.Split("/")[3] + ".txt",8 ,true, true);
				file.writeLine("\n");
				file.writeLine("Request url: " + url);
				file.writeLine("Request header: " + oSession.oRequest.headers);
				file.writeLine("Request body: " + oSession.GetRequestBodyAsString());
